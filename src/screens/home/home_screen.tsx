...
const HomeScreen: React.FC = () => {
  ...
  const lastConnectionTokenRef = useRef<string | null>(null);
  const carAssetsLoadedRef = useRef<Record<string, boolean>>({});
  const carAssetsPendingRef = useRef<Record<string, boolean>>({});
  const countdownInterval = useRef<NodeJS.Timeout | null>(null);
  const notificationAnim = useRef(new Animated.Value(-100)).current;
  const connectionInitializedRef = useRef<boolean>(false);
  const initialLiveCarsRequestedRef = useRef<boolean>(false);
  ...
  }, [routeParams?.token, connectWebSocket]);
  ...
  // ✅ FIXED: Once WebSocket is connected, request live cars once via WebSocket
  useEffect(() => {
    if (isConnected) {
      if (!initialLiveCarsRequestedRef.current) {
        initialLiveCarsRequestedRef.current = true;
        console.log(
          '🔁 WebSocket connected in HomeScreen, requesting live cars...',
        );
        getLiveCars();
      }
    } else {
  ...
  const onRefresh = async () => {
    ...
    try {
      if (isConnected) {
        console.log('📡 Requesting live cars via WebSocket...');
        getLiveCars();
      } else if (token) {
  ...
  const fetchCarImageAndDetails = async (
    ...
    try {
      ...
      console.error('Error fetching car image and details:', error);
      return false;
    }
  };
  ...
  useEffect(() => {
    filteredLiveCars.forEach(car => {
      ...
      const assetKey = `${beadingId}-${bidId}`;
      const hasImage = !!(carImageData[bidId] || carImageData[beadingId]);
      const hasDetails = !!carDetailsData[bidId];

      if (hasImage && hasDetails) {
        carAssetsLoadedRef.current[assetKey] = true;
        return;
      }

      if (
        carAssetsLoadedRef.current[assetKey] ||
        carAssetsPendingRef.current[assetKey]
      ) {
        return;
      }

      carAssetsPendingRef.current[assetKey] = true;
      fetchCarImageAndDetails(beadingId, bidId)
        .then(success => {
          if (success) {
            carAssetsLoadedRef.current[assetKey] = true;
          } else {
            delete carAssetsLoadedRef.current[assetKey];
          }
        })
        .finally(() => {
          delete carAssetsPendingRef.current[assetKey];
        });
    });
  }, [filteredLiveCars]);
  ...
  const openBidModal = async (bidCarId: string) => {
    try {
      // Find the car to get its userId (owner)
      const car = filteredLiveCars.find(
        c => c.id === bidCarId || c.bidCarId === bidCarId,
      );
      ...
      setSelectedCar({bidCarId, price: currentPrice, carUserId});
      // Set initial bid to current price + 2000, but don't auto-update it
  ...
  useEffect(() => {
    if (modalVisible && selectedCar) {
      ...
      // Update the cached price and selectedCar price, but don't auto-update the bid amount
      if (currentPrice > 0 && selectedCar.price !== currentPrice) {
        setBidModalPriceCache(prev => ({
          ...prev,
          [selectedCar.bidCarId]: currentPrice,
        }));
  ...
  const handlePlaceBid = async () => {
    ...
    const carOwnerId = selectedCar.carUserId
      ? Number(selectedCar.carUserId)
      : null;

    if (carOwnerId && currentUserId === carOwnerId) {
      Alert.alert(
        "Can't Place Bid",
        "You can't place a bid because you set the car for the bid.",
      );
      return;
    }
    ...
    // Only check if bid is less than or equal to zero, allow higher bids
    if (bidValue <= cachedPrice) {
      Alert.alert(
        "Can't Reduce the Real Amount",
        `You cannot place a bid lower than or equal to the current bid amount (₹${cachedPrice.toLocaleString()}). Please enter a higher amount.`,
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        let errorDetails = 'Unable to place bid. Please try again.';
        ...
        const errorData =
          ...
          data?.object?.error ||
          '';
        const errorDataLower = String(errorData).toLowerCase();
        ...
        // Priority 1: Check if user is trying to bid on their own car
        if (
          combinedErrorText.includes('owner') ||
          combinedErrorText.includes('your own') ||
          combinedErrorText.includes('same user') ||
          combinedErrorText.includes('cannot bid') ||
          combinedErrorText.includes('you cannot') ||
          combinedErrorText.includes('not allowed') ||
          response.status === 403
        ) {
          errorMessage = "Can't Place Bid";
          errorDetails =
            "You can't place a bid because you set the car for the bid.";
        }
        // Priority 2: Check if bid amount is too low (only for lower bids, not higher)
        else if (
          (combinedErrorText.includes('amount') &&
            combinedErrorText.includes('lower')) ||
          (combinedErrorText.includes('amount') &&
            combinedErrorText.includes('less')) ||
          combinedErrorText.includes('minimum') ||
          combinedErrorText.includes('must be greater') ||
          (response.status === 400 && combinedErrorText.includes('lower'))
        ) {
          errorMessage = "Can't Reduce the Real Amount";
          errorDetails =
            'You cannot place a bid lower than or equal to the current bid amount. Please enter a higher amount.';
        }
        // Priority 3: Show server error message if available
        else {
          const serverMessage =
            data?.message ||
            data?.error ||
